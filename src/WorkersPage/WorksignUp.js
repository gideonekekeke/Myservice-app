...
const theBase = app.firestore().collection("Plumber");
function WorksignUp() {
  ...
  const SignupPlumber = async () => {
    const newUser = await app
      .auth()
      .createUserWithEmailAndPassword(email, password);
    theBase.doc(newUser.user.uid).set({
      fullname,
      phoneNumber,
      avatar: await profile,
      email,
